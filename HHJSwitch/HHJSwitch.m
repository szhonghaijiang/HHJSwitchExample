...
typedef enum {
    HHJSwitchThumbJustifyLeft,
    HHJSwitchThumbJustifyRight
} HHJSwitchThumbJustify;

@interface HHJSwitchThumb : UIView
@property (nonatomic, assign) BOOL isTracking;
-(void) growThumbWithJustification:(HHJSwitchThumbJustify) justification;
-(void) shrinkThumbWithJustification:(HHJSwitchThumbJustify) justification;

@end

@interface HHJSwitchTrack : UIView
...
@interface HHJSwitch () <UIGestureRecognizerDelegate>
@property (nonatomic, strong) HHJSwitchTrack* track;
@property (nonatomic, strong) HHJSwitchThumb* thumb;
...
- (id)initWithFrame:(CGRect)frame
{
    self = [super initWithFrame:[HHJSwitch getThekHeightWidthRatioFrame:frame]];
    if (self) {
        [self configureSwitch];
    }
    return self;
...
-(void) setFrame:(CGRect)frame {
    if (self.shouldConstrainFrame) {
        [super setFrame: [HHJSwitch getThekHeightWidthRatioFrame:frame]];
    }
    else [super setFrame: frame];
}

+ (CGRect) getThekHeightWidthRatioFrame:(CGRect)frame {
    CGFloat width = frame.size.height*kHeightWidthRatio;
    if (frame.size.width > width) {
        return CGRectMake(frame.origin.x, frame.origin.y, frame.size.height*kHeightWidthRatio, frame.size.height);
    }
    return CGRectMake(frame.origin.x, frame.origin.y, frame.size.width, frame.size.width/kHeightWidthRatio);
}
...
-(void) configureSwitch {
    ...
    // Initialization code
    if (!_track) {
        _track = [[HHJSwitchTrack alloc] initWithFrame: self.bounds
    ...
    if (!_thumb) {
        _thumb = [[HHJSwitchThumb alloc] initWithFrame:CGRectMake(kThumbOffset, kThumbOffset, self.bounds.size.height - 2 * kThumbOffset, self.bounds.size.height - 2 * kThumbOffset)];
...
-(void) setThumbIsTracking:(BOOL)isTracking {
    if (isTracking) {
        //Grow
        [self.thumb growThumbWithJustification: self.isOn ? HHJSwitchThumbJustifyRight : HHJSwitchThumbJustifyLeft];
    }
    else {
        //Shrink
        [self.thumb shrinkThumbWithJustification: self.isOn ? HHJSwitchThumbJustifyRight : HHJSwitchThumbJustifyLeft];
    }
...
-(void) growThumbWithJustification:(HHJSwitchThumbJustify) justification {
    ...
    if (justification == HHJSwitchThumbJustifyRight) {
...
@interface HHJSwitchTrack ()
@property (nonatomic, strong) UIView* contrastView;
@property (nonatomic, strong) UIView* onView;
@end

@implementation HHJSwitchTrack
